...
The video combines the command's output with an expanded file inventory, before/after content hashes, contract-field comparisons and receipt identifiers from the same demo fixtures. Repeated headings are removed. It opens directly on RUMZO, with lines appearing progressively and no typed command introduction. At 16 seconds the view begins a smooth downward scroll through the detailed output; at 27 seconds it scrolls back to the beginning. The RUMZO mascot and final holding time were added for presentation. The DEMO label stays fixed above the scrolling content. This is an edited terminal demonstration, not a live-chain recording. It has no soundtrack or voiceover.

Format: H.264 MP4, 1920 × 1080, 30 fps, 34 seconds, with fast-start playback. The poster shows the return to the top of the transcript. The video and poster use only RUMZO assets and demo data.
