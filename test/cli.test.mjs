...
test('CLI help explains local use and exits successfully without provider access', () => {
  ...
  assert.match(result.stdout, /Watch the evidence/);
  assert.match(result.stdout, /--before/);
  assert.match(result.stdout, /watch start/);
});

test('CLI rejects malformed arguments and invalid commands before provider access', () => {
  for (const args of [['unknown'], ['serve', '--port', '0'], ['inspect', '--format', 'csv'], ['inspect', '--repo'], ['inspect', '--repo', 'a/b', '--repo', 'c/d'], ['diff'], ['watch', 'run'], ['watch', 'start', '--poll', '1']]) {
